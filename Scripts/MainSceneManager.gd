...
func connected_to_server():
	...
	send_player_info.rpc(multiplayer.get_unique_id(), player_inventory)
...
@rpc("any_peer")
func send_player_info(id, inventory):
	...
	if multiplayer.is_server():						# If the function is being called by the host server
		for player_id in GlobalVars.players_ref:	# Send player's information to all other players
			send_player_info.rpc(player_id, inventory)
...
func _on_host_button_down():
	...
	send_player_info(multiplayer.get_unique_id(), player_inventory)
	$CanvasLayer.hide()
